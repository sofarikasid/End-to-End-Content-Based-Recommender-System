...
user_profile= pd.read_csv("mylib/user_profile_test.csv", index_col="article_id").drop('Unnamed: 0', axis=1)
...
def test_run_inference_on_new_user():
    ...
    suggested_recommendations = run_inference_on_new_user(items, data, k=10, new_user_items=new_user_items)
    #assert [ 27 ,  20,  133,   30, 1131, 3720, 3374,  159, 2658,  990] ==(suggested_recommendations)
    assert np.any(np.array(suggested_recommendations) == [ 27 ,  20,  133,   30, 1131, 3720, 3374,  159, 2658,  990])

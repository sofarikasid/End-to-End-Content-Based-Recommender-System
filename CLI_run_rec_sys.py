...
def open_csv_file(tfidf_file, items_file, user_profile_file):
    ...
    user_profile = pd.read_csv(user_profile_file, index_col="article_id").drop("Unnamed: 0", axis=1)
    
    return data_tfidf, items_, user_profile


def run_recommender_cli(tfidf_file, items_file, user_profile_file, k=10, batch_size=1000):
    """
    This function trains a recommender on a batch of users and returns k recommendations.
    """
    data_tfidf, items_, user_profile = open_csv_file(tfidf_file, items_file, user_profile_file)
...
def run_inference_cli(tfidf_file, items_file, user_profile_file, k=10, new_user_items=None):
    """
